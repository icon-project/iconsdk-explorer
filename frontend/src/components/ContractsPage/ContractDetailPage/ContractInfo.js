...
import { CopyButton, TransactionLink, LoadingComponent, QrCodeButton, AddressLink } from 'components'
    ...
    render() {
        ...
        const Contents = () => {
            if (loading) {
                ...
                const { address, balance, createTx, creator, ircVersion, status, symbol, txCount, depositInfo, tokenName } = data
                const isCreator = isValidData(creator)
                const isCreateTx = isValidData(createTx)
                const { availableDeposit, availableVirtualStep } = depositInfo || {} // TODO
                return (
